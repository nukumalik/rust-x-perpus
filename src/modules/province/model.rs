...
pub mod ProvinceModel {
    use super::super::{entity::CreateProvincePayload, schema::ProvinceSchema};
    ...
    pub async fn select(
    ...
    ) -> Result<Vec<ProvinceSchema>, Error> {
        let mut sql = "SELECT * FROM provinces ".to_owned();
        if filter.is_some() {
        ...
        query_as::<Sqlite, ProvinceSchema>(sql.as_str())
            .fetch_all(pool)
            .await
    }

    pub async fn detail(pool: &Pool<Sqlite>, id: &String) -> Result<ProvinceSchema, Error> {
        query_as::<Sqlite, ProvinceSchema>("SELECT * FROM provinces WHERE id=?")
            .bind(id)
            .fetch_one(pool)
            .await
    }
    ...
    pub async fn insert(
        ...
        .bind(country.country_id().to_string())
    ...
    pub async fn update(
        pool: &Pool<Sqlite>,
        id: &String,
        country: &CreateProvincePayload,
    ) -> Result<SqliteQueryResult, Error> {
        query::<Sqlite>("UPDATE provinces SET name=?, country_id=? WHERE id=?")
            .bind(country.get_name().to_string())
            .bind(country.get_country_id().to_string())
            .bind(id)
            .execute(pool)
            .await
    }
    ...
    pub async fn delete(pool: &Pool<Sqlite>, id: &String) -> Result<SqliteQueryResult, Error> {
        query::<Sqlite>("DELETE FROM provinces WHERE id=?")
            .bind(id)
            .execute(pool)
            .await
    }
